...
(function($) {
	...
	$.fn.filterTable = function(options) { // define the filterTable plugin
		...
		var doFiltering = function(table, q) { // handle the actual table filtering
				var tbody = table.find(settings.tbodySelector); // cache the tbody element
				if (q === '' || q.length < settings.minChars) { // if the filtering query is blank or the number of chars is less than the minChars option
					tbody.find(settings.rowSelector).show().addClass(settings.visibleClass); // show all rows
					tbody.find(settings.cellSelector).removeClass(settings.highlightClass); // remove the row highlight from all cells
					if (settings.hideTFootOnFilter) { // show footer if the setting was specified
						table.find(settings.tfootSelector).show();
					}
				} else { // if the filter query is not blank
					var all_tds = tbody.find(settings.cellSelector);
					tbody.find(settings.rowSelector).hide().removeClass(settings.visibleClass); // hide all rows, assuming none were found
					...
					if (settings.hideTFootOnFilter) { // hide footer if the setting was specified
						table.find(settings.tfootSelector).hide();
					}
					if (settings.ignoreColumns.length) {
						...
						tds.each(function() {
							...
							if ($.inArray(col, settings.ignoreColumns) === -1) {
								...
								t.addClass(settings.highlightClass).closest(settings.rowSelector).show().addClass(settings.visibleClass);
							}
						});
					} else {
						if (settings.ignoreClass) {
						...
						all_tds.filter(':'+settings.filterExpression+'("'+q+'")').addClass(settings.highlightClass).closest(settings.rowSelector).show().addClass(settings.visibleClass); // highlight (class=alt) only the cells that match the query and show their rows
					}
				}
		...
		return this.each(function() {
			var t = $(this), // cache the table
				tbody = t.find(settings.tbodySelector), // cache the tbody
			...
			if (/* t[0].nodeName === 'TABLE' && */tbody.length > 0 && (settings.minRows === 0 || (settings.minRows > 0 && tbody.find(settings.rowSelector).length >= settings.minRows)) && !t.prev().hasClass(settings.containerClass)) { // only if object is a table and there's a tbody and at least minRows trs and hasn't already had a filter added
